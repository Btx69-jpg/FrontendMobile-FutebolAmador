package com.example.amfootball.ui.navigation.objects
...
/**
 * Interface que define o contrato base para todas as rotas de navegação da aplicação.
 *
 * Garante que qualquer Enum de rotas (Geral, User, Team) fornece os metadados necessários
 * para a construção automática de componentes de UI como TopAppBar e BottomNavigation.
 *
 * @property route A string única que identifica o destino no NavGraph (ex: "home", "profile/{id}").
 * @property labelResId O ID do recurso de string para o título visível na UI.
 * @property icon O ícone vetorial associado à rota.
 * @property contentDescription O ID do recurso de string para acessibilidade (TalkBack).
 * @property haveBackButton Indica se esta rota deve exibir a seta de "Voltar" na TopBar.
 */
...
/**
 * Objeto central que agrupa todas as definições de rotas da aplicação, organizadas por domínio.
 */
object Routes {
    /**
     * Nome do argumento dinâmico utilizado na rota de Chat Individual.
     * Valor: "chatRoomId"
     */
    val chatRoomId = "chatRoomId"

    /**
     * Rotas de uso geral e transversal na aplicação.
     * Inclui definições, preferências e ecrãs principais públicos (Home, Leaderboard).
     */
    ...
    /**
     * Rotas relacionadas com funcionalidades focadas no Jogador individual.
     * Inclui listagens de jogadores, equipas, chats e gestão de pedidos pessoais.
     */
    enum class PlayerRoutes(
        ...
        PLAYER_LIST(
            "playerlist",
            R.string.item_list_players,
            Icons.Default.Group,
            R.string.item_list_players_description
        ),
        TEAM_LIST(
            "teamlist",
            R.string.item_list_team,
        ...
        CHAT_LIST(
            ...
            Icons.Default.Groups3,
            R.string.item_list_chat_description
        ),
        ...
        SINGLE_CHAT(
            "chat/{${chatRoomId}}",
            R.string.single_chat,
            Icons.Default.Person,
            R.string.single_chat_description,
            true
        )
    }

    /**
     * Rotas relacionadas com a gestão de identidade do utilizador (Auth).
     * Inclui Login, Registo, Perfil e Logout.
     */
    ...
    /**
     * Rotas complexas relacionadas com a gestão e operações de uma Equipa.
     * Inclui CRUD de equipa, gestão de membros, calendário, convites e gestão de jogos.
     */
    enum class TeamRoutes(
        ...
        UPDATE_TEAM(
            "UpdateTeam",
            R.string.navbar_update_team,
            Icons.Default.Sync,
            R.string.description_navbar_update_team,
            haveBackButton = true
        ),
        HOMEPAGE(
            "HomePageTeam",
        ...
        SEARCH_PLAYERS_WITH_OUT_TEAM(
            "SearchPlayersWithoutTeam",
            R.string.navbar_players_without_team_list,
            Icons.Default.Person,
            R.string.description_navbar_players_without_team_list
        ),
        ...
        NEGOCIATE_MATCH_INVITE(
            "NegociateMatchInvite",
            R.string.negotiate_button,
            Icons.Default.EditCalendar,
            R.string.negotiate_button_description,
            haveBackButton = true
        )
    }

    /**
     * Subconjunto de rotas que compõem a Barra de Navegação Inferior (Bottom Navigation Bar).
     *
     * Estas rotas são atalhos para os ecrãs mais importantes e atuam como "Top Level Destinations".
     * Reutilizam definições dos outros Enums para evitar duplicação de constantes.
     */
