...
class ListTeamViewModel: ViewModel() {
    ...
    private val filterState = mutableStateOf(value = FiltersListTeamDto())

    private val listRanks = mutableStateOf(value = emptyList<RankNameDto>())

    //Getters
    val name: State<String> = derivedStateOf {
        filterState.value.name ?: ""
    }

    val rank: State<String> = derivedStateOf {
        filterState.value.rank ?: ""
    }

    val city: State<String> = derivedStateOf {
        filterState.value.city ?: ""
    }

    val minPoint: State<Int?> = derivedStateOf {
        filterState.value.minPoint
    }

    val maxPoint: State<Int?> = derivedStateOf {
        filterState.value.maxPoint
    }

    val minAge: State<Int?> = derivedStateOf {
        filterState.value.minAge
    }

    val maxAge: State<Int?> = derivedStateOf {
        filterState.value.maxAge
    }

    val minNumberMembers: State<Int?> = derivedStateOf {
        filterState.value.minNumberMembers
    }

    val maxNumberMembers: State<Int?> = derivedStateOf {
        filterState.value.maxNumberMembers
    }
